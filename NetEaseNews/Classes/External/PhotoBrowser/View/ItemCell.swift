...
class ItemCell: UICollectionViewCell {
    ...
    var photoModel: PhotoModel!{didSet{dataFill()}}
    ...
    var showVC: PhotoBrowser!
    
    var oriPoint = CGPointZero
...
extension ItemCell: UIScrollViewDelegate{
    ...
    override func awakeFromNib() {
        ...
        panGesture = UIPanGestureRecognizer(target: self, action: #selector(ItemCell.panGesture(_:)))
...
//        scrollView.layer.borderWidth = 5
    ...
    @objc private func panGesture(sender: UIPanGestureRecognizer) {
        ...
        if isFirstPan {
            ...
            bgShadowView = UIView(frame: UIScreen.mainScreen().bounds)
            UIApplication.sharedApplication().keyWindow!.window?.addSubview(bgShadowView)
            bgShadowView.backgroundColor = UIColor.blackColor()
            bgShadowView.alpha = 0.2
            UIApplication.sharedApplication().keyWindow!.window?.insertSubview(bgShadowView, belowSubview: showVC.view)
           
            UIView.animateWithDuration(0.5, animations: {
                self.showVC.view.backgroundColor = UIColor.blackColor().colorWithAlphaComponent(0)
//                UIApplication.sharedApplication().statusBarHidden = false
//                if self.vc.navigationController != nil {self.vc.navigationController?.navigationBarHidden = false}
            })
            isFirstPan = false
        }
        showVC.isHiddenBar = true
        showVC.dismissBtn.hidden = true
        showVC.saveBtn.hidden = true
        bottomContentView.hidden = true
        let location = sender.translationInView(sender.view)
        if oriPoint == CGPointZero {
            oriPoint = self.imageV.frame.origin
        }
        self.imageV.transform = CGAffineTransformTranslate(self.imageV.transform, location.x, location.y)
        sender.setTranslation(CGPointZero, inView: sender.view)

        if sender.state == .Ended {
            UIView.animateWithDuration(0.5, animations: {
                self.bgShadowView.alpha = 0
            }, completion: { (_) in
                self.bgShadowView.removeFromSuperview()
                self.showVC = nil
            })
            dismiss?()
        }
